...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      const q = "SELECT * FROM `product_video` WHERE product_id = ?";
  ...
  if (req.method == "DELETE") {
    try {
      ...
      // get productThumbnail data
      const [productThumbnail] = await conn.query(
        "SELECT video_thumbnail FROM product_video WHERE prod_video_id = ?",
        [id]
      );

      // Query for delete Product Vedio
      const q = "DELETE FROM product_video WHERE prod_video_id = ?";
    ...
    try {
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        const [category] = await conn.query(
          "SELECT category_image, category_icon FROM product_video WHERE prod_video_id = ?",
          [id]
        );
        ...
        if (!files.category_icon && !files.category_image) {
          ...
          sql =
            "UPDATE `product_video` SET `category_title`= ?, `category_description`= ?, `meta_tag`= ?, `meta_description`= ?, `meta_keyword`= ?, `canonical_url`= ?, `category_image`= ?, `category_icon`= ?  WHERE prod_video_id = ?";
