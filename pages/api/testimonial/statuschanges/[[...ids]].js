...
export default async function handler(req, res) {
  // Handling PATCH request for updating testimonial status
  if (req.method == "PATCH") {
    try {
      ...
      if (status == 1) {
        // If the status is 1, update the testimonial status to 0
        const sql = "UPDATE testimonial SET status=0 WHERE id = ?";
        const [rows] = await conn.query(sql, [id]);

        res.status(200).json(rows);
      } else {
        // If the status is not 1, update the testimonial status to 1
